...
interface SetThresholdsTaskArgs {
  pooladdress: string;
  sourcechain: string;
  amounts: string;
...
task("setThresholds", "Sets thresholds")
  ...
  .setAction(async (taskArgs: SetThresholdsTaskArgs, hre) => {
    const {
    ...
    try {
      const { BurnMintTokenPoolWithApproval__factory } = await import("../typechain-types");
      const signer = (await hre.ethers.getSigners())[0];

      const tx = await BurnMintTokenPoolWithApproval__factory.connect(
        pooladdress,
        signer
      ).setThresholds(sourcechain, parsedAmounts, parsedNumOfApprovers);
